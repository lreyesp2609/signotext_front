...
const GenerarModelo = () => {
  ...
  const generarModelo = async () => {
    try {
      ...
      setLoading(false);
      alert("Modelo generado con éxito");
    } catch (error) {
      setLoading(false);
      alert("Error al generar el modelo");
      console.error(error);
    }
  };
  ...
  return (
    <Card className="mt-6 w-96 mx-auto">
      ...
      <CardBody>
        <Card.Title>Generar Modelo</Card.Title>
        <Card.Text>
          El modelo se genera a partir de las imágenes almacenadas en el RPC.
        </Card.Text>
      </CardBody>
      <CardFooter>
